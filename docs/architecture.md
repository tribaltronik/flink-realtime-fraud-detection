...
![Approach 1](./approach1.svg)

### Approach 2: Apache Flink Cluster

![Approach 2](./approach2.svg)

> Open `.d2` files in [D2](https://d2lang.com) or `.excalidraw` files in [Excalidraw](https://excalidraw.com) to edit diagrams.
...
3. Submits the fraud detection SQL job via `submit-job.sh`
...
# Watch suspicious transactions (Kafka)
docker exec docker-kafka-1 kafka-console-consumer --topic suspicious-transactions --from-beginning --bootstrap-server localhost:9092
